...
from fa_lime import FeatureAttributionLIME
from fa_shap import FeatureAttributionSHAP
from fa_intg import FeatureAttributionIntG
from sklearn.base import ClassifierMixin
import copy
import util
from discern_base import DisCERN
import numpy as np
...
class DisCERNTabular(DisCERN):
    ...
    def find_cf(self, test_instance, test_label, cf_label='opposite'):
        ...
        # 대신 목표하는 타겟 점수(예: 현재 점수의 80%)를 설정하도록 변경
        target_score = test_label * 0.8 
        nun_data, nun_label = util.nun(self.train_data, self.train_labels, norm_test_instance, test_label, target_score)    

        _weights = self.feature_attrib.explain_instance(norm_test_instance, query_label=test_label, nun=nun_data)

        if _weights is None:
            # SHAP 실패 시: (이웃 데이터 - 현재 데이터)의 절대값을 가중치로 임시 사용
            diff = np.abs(nun_data - norm_test_instance)
            _weights = [(i, w) for i, w in enumerate(diff)]
        
        print(f"DEBUG - weights: {_weights}")
        if _weights is None:
            print("Error: feature_attrib.explain_instance가 None을 반환했습니다.")
        ...
        while True:
            ...
            if isinstance(self.model, ClassifierMixin):
                ...
                # [추가] 일반적인 사이킷런 기반 모델(XGB, LGBM 등)을 위한 범용 예측 코드
                try:
                    new_label = self.model.predict(np.array([x_adapted]))[0]
                except Exception:
                    # 확률값으로 나올 경우를 대비한 예외 처리
                    preds = self.model.predict(np.array([x_adapted]))
                    new_label = np.argmax(preds, axis=-1)[0] if len(preds.shape) > 1 else preds[0]
            print('new_label: ', new_label, 'nun_label: ', nun_label, 'test_label: ', test_label)
            if new_label < test_label: # 현재 단계보다 낮아지기만 하면 성공!
                if sparsity > 0:
            ...
            # [133행 부근 수정] 모든 피처를 다 검토했을 때 에러 대신 결과 반환
            if now_index >= len(self.feature_names):
                print("⚠️ 경고: 완전한 개선안을 찾지 못해 가장 근접한 상태를 반환합니다.")
                if sparsity > 0:
                    proximity = proximity / sparsity
                return x_adapted, new_label, sparsity, proximity # Exception 대신 return
